...
export const crearProyecto = async (req, res, next) => {
    try {
        const proyecto = await proyectoService.crearProyecto(req.body, req.user.id);
...
export const actualizarProyecto = async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        const proyecto = await proyectoService.ActualizarProyecto(id, req.body, req.user.id);
        res.status(200).json({
            ok: true,
...
export const eliminarProyecto = async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        await proyectoService.eliminarProyecto(id, req.user.id);
        res.status(200).json({
            ok: true,
